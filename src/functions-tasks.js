...
function getPolynom(...coefficients) {
  ...
  return (x) => {
    ...
    for (let i = coefficients.length - 1, y = 0; i >= 0; i -= 1, y += 1) {
      result += coefficients[i] * x ** y;
    }
...
function logger(func, logFunc) {
  return (...args) => {
    const functionCallAsString = `${func.name}(${args.map(JSON.stringify).join()})`;

    logFunc(`${functionCallAsString} starts`);

    const result = func(...args);

    logFunc(`${functionCallAsString} ends`);
